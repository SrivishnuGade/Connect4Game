...
Label(root, text = 'Time', fg = 'DarkOliveGreen1', bg = 'grey11',font =  ('Copper Black', 25)).place(x = 200, y = 190)
...
player2_entry_name = None
time_var = IntVar()
menu = OptionMenu(root, time_var, *[30,60,90,120,150,180,300,600])

time_var.set(300)
menu.place(x = 220, y = 260)
menu.config(bg = 'grey11', fg = 'white')
...
def create_widgets():
    ...
    gt.start()
    board.bind('<B1-Motion>', move)
    #creating empty list to store grid
    l=[]
    for i in range(6):
        l.append([])
        for j in range(7):
            l[i].append(0)
    #creating empty list to store tagnames in position on grid
    ltn=[]
    for i in range(6):
        ltn.append([])
        for j in range(7):
